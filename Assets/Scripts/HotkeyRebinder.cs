...
using UnityEngine;
using UnityEngine.UI; //Alow UI elements
using TMPro; //Allows to use TeshMeshPro
...
public class HotkeyRebinder : MonoBehaviour
{
    ...
    public Color32 changedKey = new Color32(39, 171, 249, 255);              //When we click a key rebind button, make it orange. Feedback to the player.
    public Color32 selectedKey = new Color32(239, 116, 36, 255);            //WHen we've rebinded it, make it blue. More feedback.
    ...
    public void SaveKeys()
    {
        ...
        {
            PlayerPrefs.SetString(key.Key, key.Value.ToString());               //Save to PlayerPrefs for each key
        }
    ...
    public void ChangeKey(GameObject clickedKey)                    //Function for a button when we select it.
    {
        ...
        if(clickedKey != null)
        {
            Image image = currentKey.GetComponent<Image>();
            if(image != null)
            {
                image.color = selectedKey;   //Just visual aid
            }
    ...
    private void OnGUI()    //Allows us to run events. In this dase, key presses.
    {
        ...
        if (currentKey != null) //this will fix issues later
        {
